...
const NOTIFICATION_NAME = "RemindMissingWorkerDuration";

export const renderWorkerReminderEmail = async ({
  locale,
  businessUnit,
  ...vars
}) => {
  ...
  const parts = {
    subject: renderMessage(
      NOTIFICATION_NAME,
      locale,
      "notifications.RemindMissingWorkerDuration.email.subject",
      translationVars
    ),
    content: renderMessage(
      NOTIFICATION_NAME,
      locale,
      "notifications.RemindMissingWorkerDuration.email.content",
      translationVars
    ),
    managehours: renderMessage(
      NOTIFICATION_NAME,
      locale,
      "notifications.RemindMissingWorkerDuration.email.managehours",
      translationVars
    ),
  };
